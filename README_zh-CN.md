...
在落地 `tutuicmptunnel` 之前，先确认当前链路是否适合承载 ICMP 隧道。核心思路是：对比 ICMP 与 UDP 在同一链路下的实际表现。
...
那么通常说明这条链路具备部署 `tutuicmptunnel` 的价值，可以继续推进。
...
然后重复第 1 步测试。只有在排除这些中间设备影响后，才能更准确判断该链路是否适合部署 `tutuicmptunnel`。
